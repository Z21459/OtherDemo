...
local port = tonumber(arg[arg.n])--转换成数字
...
local function docmd(cmdline, print, fd)--解析命令  调用不同函数
...
local function console_main_loop(stdin, print)
	...
	local ok, err = pcall(function()
		while true do
		-- 使用死循环 监听命令端的命令 读取控制台输入函数
			local cmdline = socket.readline(stdin, "\n")
			if not cmdline then
				break
			end
			if cmdline:sub(1,4) == "GET " then--获取1 -4 个命令
				-- http
			...
			--对nc或者telent命令进行解析
			if cmdline ~= "" then
				docmd(cmdline, print, stdin)--docmd进行命令解析 调用具体执行接口
			end
		end
	end)
	if not ok then
...
function COMMANDX.debug(cmd)
	...
	local ok, err = skynet.call(agent, "lua", "start", address, cmd.fd)--进入到这个函数里面
	stop = true
	if term_co then
...
function COMMAND.trace(address, proto, flag)
	...
	skynet.call(address, "debug", "TRACELOG", proto, flag)--调用debug.lua 里面的函数
end
