...
local function hook_dispatch(dispatcher, resp, fd, channel)--这个怎么出现了空值
	change_prompt(string.format(":%08x>", skynet.self()))
	...
	func = replace_upvalue(dispatcher, HOOK_FUNC, hook)--代替新值
	if func then
		local function idle()
...
function M.start(import, fd, handle)
	...
	local channel = debugchannel.connect(handle)---获取read函数   creat获取write函数
